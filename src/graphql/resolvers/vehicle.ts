...
export const vehicleResolvers = {
  Query: {
    ...
    customVehicleType: async (_: any, args: { id: string }, { db, user }: Context) => {
      ...
      const [found] = await db
        .select()
        .from(cvtTable)
        .where(and(eq(cvtTable.id, args.id), eq(cvtTable.account_id, user!.account_id)))
        .limit(1)
      return found ?? null
    },
    ...
    vehicle: async (_: any, args: { id: string }, { db, user }: Context) => {
      ...
      const [found] = await db
        .select()
        .from(vehicleTable)
        .where(and(eq(vehicleTable.id, args.id), eq(vehicleTable.account_id, user!.account_id)))
        .limit(1)
      return found ?? null
    }
  },
  Mutation: {
    ...
    updateVehicle: async (_: any, args: any, { db, user }: Context) => {
      ...
      if (args.vehicle_type !== undefined) updates.vehicle_type = args.vehicle_type
      if (args.depot_id !== undefined) updates.depot_id = args.depot_id ?? null
      if (args.data !== undefined) updates.data = args.data
      if (args.comment_for_account !== undefined) updates.comment_for_account = args.comment_for_account
      updates.updated_at = Math.floor(Date.now() / 1000)
      const [updated] = await db
  ...
  Vehicle: {
    vehicleTypeInfo: (parent: { vehicle_type: string }, _: any, { db }: Context) =>
      db.select().from(cvtTable).where(eq(cvtTable.id, parent.vehicle_type)).limit(1)
        .then(r => r[0] ?? null),
    depot: (parent: { depot_id: string | null }, _: any, { db }: Context) => {
